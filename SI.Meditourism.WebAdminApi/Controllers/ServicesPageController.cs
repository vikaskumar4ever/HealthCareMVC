...
namespace SI.Meditourism.WebAdminApi.Controllers
{
    ...
    public class ServicesPageController : Controller
    {
        ...
        [HttpGet("{PageNo}/{PageSize}/{serviceId}")]
        public ServicePage Get(int PageNo, string PageSize, Guid? ServiceId)
        {
            int iPageSize = Convert.ToInt32(PageSize);
            ServicePage objReturn = new ServicePage();
            using (Service_BAL objBAL = new Service_BAL())
            {
                objReturn = objBAL.GetRecordsPage(PageNo, iPageSize, ServiceId);
            }
            return objReturn;
        }
